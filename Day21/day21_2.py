...
dir_length = {key: len(value[0]) for key, value in dir_seqs.items()}

@cache
def compute_length(seq, depth=25):
    if depth == 1:
        return sum(dir_length[(x, y)] for x, y in zip("A" + seq, seq))
    length = 0
...
for line in open("Day21\input.txt").read().splitlines():
